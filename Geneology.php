...
<div class="container" >
		...
		<div id="body">
			<center>
				<div id="box-parent">
					<div id="user_parent" class="user empty" data-number="Admin"><div id="circle">1</div>
					
					<div id="resultAdmin"></div>
						<div id="profile_container" class="profile_containerAdmin"></div>
					</div>
				</div>
				...
				<div id="box-parent" class="left top" >
					...
					<div id="result2"></div>
						<div id="profile_container" class="profile_container2"></div>
					</div>
				</div>
				<div id="space"></div>
				<div id="box-parent" class="right ">
					...
					<div id="result3"></div>
						<div id="profile_container" class="profile_container3"></div>
					</div>
				</div>
				<br>
				...
				<div id="box-third" class="left" style="margin-left: -600px;">
					...
					<div id="result4"></div>
						<div id="profile_container" class="profile_container4"></div>
					</div>	
					<div id="user_parent" class="user right-parent empty "data-number="5"><div id="circle">5</div>
					<div id="result5"></div>
						<div id="profile_container" class="profile_container5"></div>
					</div>
				</div>
				<div id="box-third" class="right" style="margin-right: -600px;">
					...
					<div id="result6"></div>
						<div id="profile_container" class="profile_container6"></div>
					</div>
					<div id="user_parent" class="user right-parent empty" data-number="7"><div id="circle">7</div>
					<div id="result7"></div>
						<div id="profile_container" class="profile_container7"></div>
					</div>
				</div>
				<br>
				<div id="box-third" class="left" style="margin-left: -700px;margin-top: 100px;">
					...
					<div id="result8"></div>
						<div id="profile_container" class="profile_container8"></div>
					</div>
					<div id="user_parent" class="user right-parent empty" data-number="9"><div id="circle">9</div>
					<div id="result9"></div>
						<div id="profile_container" class="profile_container9"></div>
					</div>
				</div>
				<div id="box-third" class="right" style="margin-right: -700px;">
					...
					<div id="result10"></div>
					<div id="profile_container" class="profile_container10"></div>
				</div>
					<div id="user_parent" class="user right-parent empty" data-number="11"><div id="circle">11</div>
					<div id="result11"></div>
						<div id="profile_container" class="profile_container11"></div>
				</div>
				</div>
...
<div id="box-last" style="padding:0px;background: ;display:block;margin-top:150px;width:100%;margin-left: ;">
				...
				<div id="result12"></div>
					<div id="profile_container" class="profile_container12"></div>
				</div>
				<div id="user_parent" class="user empty" data-number="13"><div id="circle">13</div>
				<div id="result13"></div>
					<div id="profile_container" class="profile_container13"></div>
				</div>
				<div id="user_parent" class="user empty" data-number="14"><div id="circle">14</div>
				<div id="result14"></div>
					<div id="profile_container" class="profile_container14"></div>
				</div>
				<div id="user_parent" class="user empty" data-number="15"><div id="circle">15</div>
				<div id="result15"></div>
					<div id="profile_container" class="profile_container15"></div>
				</div>
...
function on_load_member_position(){
		$(window).on('load',function(){
			...
			$.ajax({
				...
				success:function(data){
					...
					fetch_member_information(number);
				}
			});
    	});		
	});
}
on_load_member_position();
function load_member_position(){
	$(".empty").each(function(){
    		var number = $(this).data('number');
				$.ajax({
					url:'data/fetch_member_position_and_send.php',
...
function fetch_member_information(number){
	$.ajax({
		url:'data/fetch_member_information.php',
		type:'POST',
		data:{number:number},
		success:function(data){
			$(".profile_container"+number).html(data);
		}
	});
}
...
if(number != 1){
	   switch (event.which) {
        case 1:
            ...
            $(document).on('click','.add_position'+number,function(e){
            	e.preventDefault();
            	var username = $(this).data('username');
            					$.confirm({
			    title: 'Confirm!',
			    content: 'Are you sure? You want to add '+username+' to this Position?',
			    buttons: {
			        cancel: function () {
			        ...
			        Archive: {
			            ...
			            action: function(){
			            	$.ajax({
			            		url:'data/add_member_position.php',
			            		type:'POST',
			            		data:{username:username,number:number},
			            		success:function(){
			            			fetch_member_position(number);
			            			load_member_position();
			            		}
			            	});
			            }
			        }
             ...
             $(document).on('click','.remove_position'+number,function(e){
            	e.preventDefault();
            	var username = $(this).data('username');
            					$.confirm({
			    title: 'Confirm!',
			    content: 'Are you sure? You want to Remove '+username+' to this Position?',
			    buttons: {
			        cancel: function () {
			        ...
			        Archive: {
			            ...
			            action: function(){
			            	$.ajax({
			            		url:'data/remove_member_position.php',
			            		type:'POST',
			            		data:{username:username,number:number},
			            		success:function(){
			            			console.log('success');
...
function fetch_member_position($number){
      $.ajax({
    	url:'data/fetch_member_position.php',
    	type:'POST',
    	data:{number:number},
...
<style type="text/css">
	...
	#body .user #profile_container #container{
		padding: 10px;
		background: #51AED9;
	}
	#body .user #profile_container #container #profile_picture{
		width: 80px;
		height: 80px;
		background: black;
		border-radius: 50%;
	}
	#body .user #profile_container #container #profile_picture img{
		width: 80px;
		height: 80px;
		border-radius: 50%;
	}
...
#right_click_container .user #description{
	padding: 5px;
	min-width: 120px;
	background: white;
	color: black;
...
#right_click_container .user:hover #description{
	display: block;
}
